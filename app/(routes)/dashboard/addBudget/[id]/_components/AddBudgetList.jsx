...
import { additionalBudget } from '@/utils/schema'
...
const AddBudgetList = ({ addBudgetList, refreshData }) => {

    const [rowData, setRowData] = useState([]);
    const [searchInput, setSearchInput] = useState();

    useEffect(() => {
        setRowData(addBudgetList);
 
    }, [addBudgetList]);

    const deleteAddBudget = async (id) => {
        try {
            await db.delete(additionalBudget)
                .where(eq(additionalBudget.id, id))
                .execute(); // 'returning' is not needed for delete operations
            toast('Expense deleted successfully');
            refreshData();
            budgetInfo();
        } catch (error) {
            console.error("Error deleting AddBudget:", error.message);
        }
    }

    const handleDeleteClick = (id) => {
        if (window.confirm("Are you sure you want to delete this AddBudget?")) {
            deleteAddBudget(id);
        }
    };
